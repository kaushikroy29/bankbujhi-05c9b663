...
const Compare = () => {
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-x-hidden">
      <Header />
      <main className="flex-1 max-w-[1280px] mx-auto container-padding py-8 w-full">
        {/* Breadcrumbs */}
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-primary text-sm font-medium">Credit Cards</span>
        </div>

        {/* Page Heading */}
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="flex flex-col gap-1">
            <h1 className="text-4xl font-black tracking-tight">Credit Card Search</h1>
            <p className="text-muted-foreground font-medium">
              Finding the best matches for your wallet in Bangladesh
            </p>
          </div>
          <div className="flex items-center gap-3 bg-card p-2 rounded-xl border border-primary/10">
            <span className="text-sm font-medium pl-2">Sort by:</span>
            <select className="bg-transparent border-none text-sm font-bold focus:ring-0 text-primary cursor-pointer">
          ...
          <aside className="w-full lg:w-72 shrink-0">
            <div className="sticky top-24 flex flex-col gap-6">
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedFilters.map((filter) => (
                      <button
                        key={filter}
                        className="flex items-center gap-1 bg-primary/10 text-primary px-3 py-1.5 rounded-lg text-xs font-bold group"
                      >
                        {filter}
                        <MaterialIcon name="close" className="text-base group-hover:scale-110" />
                      </button>
                    ))}
                  </div>
                </div>
                  ...
                  {filterCategories.map((cat) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <MaterialIcon name={cat.icon} className="text-primary" />
              ...
              <div className="bg-primary rounded-2xl p-6 text-primary-foreground overflow-hidden relative">
                ...
                <MaterialIcon 
                  name="warning" 
                  className="absolute -bottom-4 -right-4 text-primary-foreground/10 text-9xl" 
                />
              </div>
            </div>
          </aside>
